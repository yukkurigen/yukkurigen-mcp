...
2. **台本を書く**: 話者(`speaker`=キャラid)とセリフ(`text`)の配列を作る。掛け合い形式が「ゆっくり」らしい。
   ...
   - 章の見出し・強調テロップ・写真・図解は、台本から自動で付く（下の「図解は自動で入る」）。
3. **背景を決める**: `backgroundImageUrl`（https のURL）を渡す。
   ...
   **図解は自動で入る**: 焼く直前に台本から、要点・手順・数字を図にした「図解カード」と、
   話に出てくる物の**写真**（CC0・パブリックドメインのストック写真）を選び、画像にして行の**素材**
   （`material_image_url`）に入れる。あわせて左上の**章の見出し**と、要の言葉の**強調テロップ**も付く
   （`autoVisuals: false` で全部止める）。
   素材なので、あとから `get_project` で見えて、エディタの素材の列で差し替え・削除できる。自分で決めたいときは、
   その話題が始まる行に `card: { "title": "…", "items": ["…","…"], "style": "list" }`
   を書く（`steps` は番号付き、`point` は要点を1つ大きく）。1つでも書くと自動では作らない。
