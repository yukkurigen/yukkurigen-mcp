...
4. **カメラを置く**: 盛り上がり・感情のピーク・オチの行に `cameraMode: "dynamic"` を
   ...
   **2〜4回に留めること。全行に付けると寄りっぱなしになり、寄りが効かなくなる。**
   立ち絵が画面に出ている行にだけ効く。
   **表情と効果音**: 感情が動く行に `emotion`（`joy` / `surprise` / `sad` / `angry` /
   `explaining` / `love`）を付けると立ち絵の表情が変わり、表情に合う効果音も自動で鳴る
   （3行に1回まで）。自分で選びたいときは `effect`（例: `"ショック1"`, `"きらーん1"`,
   `"ひらめく1"`, `"チーン1"`）を書く。**1行でも `effect` を書くと自動の効果音は止まる。**
